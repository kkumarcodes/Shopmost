...
const { Handler } = require('@shopmost/shopmost/src/lib/middleware/Handler');
...
function watchMR() {
  ...
  if (existsSync(resolve(CONSTANTS.ROOTPATH, 'packages'))) {
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/shopmost/src/modules/*/controllers/**'
      )
    );
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/shopmost/src/modules/*/apiControllers/**'
      )
    );
  }
