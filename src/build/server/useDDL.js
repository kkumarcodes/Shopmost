...
if (existsSync(path.resolve(CONSTANTS.ROOTPATH, './build'))) {
  rmdirSync(path.resolve(CONSTANTS.ROOTPATH, './build'), {
    recursive: true
  });
}
...
const {
  createVendorConfig
} = require('../../../../lib/webpack/configProvider');
...
webpackVendorPromise.then(async () => {
  controllers.forEach((route) => {
    const buildFunc = async function () {
      ...
      content += '\r\n';
      await mkdir(
        path.resolve(CONSTANTS.ROOTPATH, './build', buildPath),
        { recursive: true }
      );
      await writeFile(
        path.resolve(
