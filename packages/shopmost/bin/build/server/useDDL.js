...
const { getRoutes } = require('@shopmost/shopmost/src/lib/router/routes');
const {
  getComponentsByRoute
} = require('@shopmost/shopmost/src/lib/componee/getComponentByRoute');
...
if (existsSync(path.resolve(CONSTANTS.ROOTPATH, './.shopmost/build'))) {
  rmdirSync(path.resolve(CONSTANTS.ROOTPATH, './.shopmost/build'), {
    recursive: true
  });
}
...
const {
  createVendorConfig
} = require('@shopmost/shopmost/src/lib/webpack/configProvider');
...
webpackVendorPromise.then(async () => {
  controllers.forEach((route) => {
    const buildFunc = async function () {
      ...
      content += '\r\n';
      await mkdir(
        path.resolve(CONSTANTS.ROOTPATH, './.shopmost/build', buildPath),
        { recursive: true }
      );
      await writeFile(
        path.resolve(
        ...
        plugins: [
          new webpack.DllReferencePlugin({
            manifest: path.resolve(
              CONSTANTS.ROOTPATH,
              './.shopmost/build/vendor-manifest.json'
            )
          })
        ]
      });
