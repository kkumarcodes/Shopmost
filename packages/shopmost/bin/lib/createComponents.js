...
const isProductionMode = require('@shopmost/shopmost/src/lib/util/isProductionMode');
const {
  getRouteBuildPath
} = require('@shopmost/shopmost/src/lib/webpack/getRouteBuildPath');
...
module.exports.createComponents = async function createComponents(
  ...
  await Promise.all(
    routes.map(async (route) => {
      ...
      `;
      if (isProductionMode()) {
        contentClient += `import Hydrate from '@shopmost/shopmost/src/lib/components/react/client/Hydrate';`;
      } else {
        contentClient += `import { App } from '@shopmost/shopmost/src/lib/components/react/client/Client';
      const hot = require('webpack-hot-middleware/client?path=/eHot/${route.id}&reload=true');
      import { HotReload } from '@shopmost/shopmost/src/lib/components/react/client/HotReload';
      `;
      }
      contentClient += '\r\n';
      ...
      if (!clientOnly) {
        ...
        contentServer += '\r\n';
        contentServer += `import Area from '@shopmost/shopmost/src/lib/components/Area';`;
        contentServer += '\r\n';
